...
const updatedMonthsList = initialMonthsList.map(curr => {
  const prevObj = initialMonthsList.find(each => each.month === curr.month - 1)
  if (curr.month === 1) {
    carry = 1
  } else {
    carry = (carry + prevObj.dates.length) % 7
  }

  carry = carry === 0 ? 7 : carry
...
const App = () => {
  ...
  const [monthsList, setMonthsList] = useState(updatedMonthsList)
  const [activeBtn, setActive] = useState(
    location.pathname === '/reports' ? 'Reports' : 'Home',
  )

  const onChangeActive = btn => {
    setActive(btn)
  }

  return (
    <DailyMoodTracker.Provider
      value={{
        monthsList,
        setMonthsList,
        emojisList,
